...
def redraw(*, reset_zoom=False):
    ...
    for label, color in label_colors.items():
        ...
        axs[1].scatter(
            subset["time"], absed_subset, c=color, s=10, label=labels_map[label]
        )

    axs[0].set_title(
        f"Label: {labels_map[current_label[0]]} (0–3 = change, u = undo, s = save, q = quit)"
    )
...
def onclick(event):
    ...
    if len(click_points) == 2:
        ...
        if not idxs.empty:
            ...
            status_message[0] = (
                f"✓ Labeled {x1:.2f}–{x2:.2f} as {labels_map[current_label[0]]}"
            )
        else:
